...
const Optimization=()=>{
    ...
    // by using useMemo react remembers the most expensive value which is returned from the function calculateMostExpensive and when ever it
    // re renders it wont call the function . it only called when product changes means the item in dependancy array changes 
    return (
        <>
        <h2>Count : {count}</h2>
        <button type='button' onClick={()=> setCount(count+1)}>click me to increase count</button>
        <h2>cart : {cart}</h2>
        <h2>most expensive price is {mostExpensive}</h2>
